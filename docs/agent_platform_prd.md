...
| 运行时核心层 | runtime, events, tasks, sessions, scheduler, workers | Agent 事件循环、任务生命周期、事件流、会话、调度、异步执行。 |
...
Runtime 是整个平台的执行内核，负责驱动 Agent 任务生命周期、事件循环、状态机、模型调用、工具调用、Hook 分发、checkpoint、replay。

### 功能点

| 编号 | 功能 | 优先级 |
|---|---|---|
| RT-001 | 创建 Root Task | P0 |
...
| EVT-005 | 支持 replay by task_id | P1 |
...
| 编号 | 模式 | 功能 | 优先级 |
|---|---|---|---|
| PAT-001 | Prompt Chaining | ChainRunner | P0 |
| PAT-002 | Routing | Rule/LLM/Semantic Router | P0 |
| PAT-003 | Parallelization | Fan-out/Fan-in | P1 |
| PAT-004 | Planning | Todo/DAG Planner | P0 |
| PAT-005 | Reflection | Critic/Verifier/Reviser | P1 |
| PAT-006 | Resource Optimization | Model/Cost/Latency Selector | P2 |
| PAT-007 | Debate | 多候选辩论 | P2 |
| PAT-008 | Self-Correction | 自动修正 | P2 |
...
Context 模块负责将 session、task、memory、rag、vfs、tool observations 组装为模型可用上下文。
...
| patterns | chaining、routing、planning 基础版 |
...
| 编号 | 验收项 | 通过标准 |
|---|---|---|
| AC-001 | 创建任务 | API 能创建 task 并返回 task_id。 |
...
| AC-008 | Trace 记录 | 每个任务有 trace_id 和 cost/token 统计。 |
| AC-009 | Memory 写入 | Summary memory 可写入、检索。 |
| AC-010 | VFS 保存产物 | Agent 生成物能保存到 /artifacts。 |
| AC-011 | Replay | 能按 task_id 查询事件轨迹。 |
| AC-012 | CLI | CLI 能运行一次本地 agent task。 |
