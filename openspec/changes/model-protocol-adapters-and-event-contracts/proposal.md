...
- 新增模型生命周期事件定义，供后续 runtime/Event Store 消费；公开执行关联键采用 `agent_run_id`，本次不实现事件持久化。
...
- 关键假设：MVP 的真实远程 provider 仍为 OpenAI；Anthropic 仅保留协议扩展边界；runtime 以 `AgentRun` 作为执行单元，工具执行消费这些契约但不在本变更实现。
...
- 不实现 Event Store、Event Bus、replay、runtime tool dispatch 或 AgentRun 聚合统计。
