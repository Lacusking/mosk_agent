...
`Request -> Access -> Identity -> Runtime Create Task -> Skill Resolve -> Context Assemble -> Prompt Render -> Model Invoke -> Tool/MCP/A2A Dispatch -> Observation Append -> Summary Memory Update -> Artifact Persist -> Event Store -> Final Response`
...
- 运行时核心层：`runtime, events, tasks, sessions, scheduler, workers`
...
- `api`：FastAPI 核心任务接口
- `cli`：Typer 本地运行与调试入口
- `runtime`：任务状态机、事件循环、step runner
- `events`：Event Sourcing、Event Store
- `tasks`：Task/TaskStep 与状态持久化
...
- 关键跨模块 schema（如 AgentRequest、Task、RuntimeEvent、ToolDefinition）优先定义在 `contracts/`
...
- Runtime 关键节点必须产生日志/事件：Task、Model、Tool、Memory、Artifact。
- 事件存储是回放与排障事实来源，不可跳过。
- 成本与 Token 使用量必须可统计。
...
- Agent：执行任务的智能体实例
- Task：一次目标驱动的运行单元
- Step：Task 内部执行步骤（model/tool/agent/workflow/human/system）
